...
public class HoaDonPanel extends JPanel {
    ...
    public HoaDonPanel(TaiKhoan taiKhoan) {
        ...
        btnTaoHoaDon.setBackground(Color.decode("#28A745"));
        btnTaoHoaDon.setForeground(Color.WHITE);
        btnXemHoaDon.setFont(buttonFont);
        btnXemHoaDon.setBackground(Color.decode("#007BFF"));
        btnXemHoaDon.setForeground(Color.WHITE);
        btnXoaHoaDon.setFont(buttonFont);
        btnXoaHoaDon.setBackground(Color.decode("#DC3545"));
        btnXoaHoaDon.setForeground(Color.WHITE);
        ...
        btnLamMoi.addActionListener(e -> loadHoaDonData());
    ...
    private void loadHoaDonData() {
        ...
        try {
            ...
            for (HoaDon hd : danhSach) {
                ...
                TaiKhoan tk = new TaiKhoanDAOImpl().getTaiKhoanById(hd.getTaiKhoanId());
                
                Object[] row = {
                    hd.getId(),
                    hd.getNgayLap(),
                    (kh != null) ? kh.getHoTen() : "Không xác định",
                    (kh != null) ? kh.getSoDienThoai() : "Không xác định",
                    hd.getTongTien(),
                    (tk != null) ? tk.getTenDangNhap() : "Không xác định"
                };
    ...
    DefaultTableModel sanPhamTableModel = new DefaultTableModel(
        ...
        @Override
        public boolean isCellEditable(int row, int column) {
            return column == 4; // Chỉ cho phép chỉnh sửa cột Số lượng mua
        }
    };
    ...
    private void xemHoaDon() {
        ...
        try {
            ...
            infoPanel.add(new JLabel("ID Hóa Đơn:"));
            infoPanel.add(new JLabel(String.valueOf(hd.getId())));
            infoPanel.add(new JLabel("Ngày Lập:"));
            infoPanel.add(new JLabel(String.valueOf(hd.getNgayLap())));
            infoPanel.add(new JLabel("Khách Hàng:"));
            infoPanel.add(new JLabel(kh != null ? kh.getHoTen() : "Không xác định"));
            infoPanel.add(new JLabel("Nhân Viên:"));
            infoPanel.add(new JLabel(tk != null ? tk.getTenDangNhap() : "Không xác định"));
            infoPanel.add(new JLabel("Tổng Tiền:"));
            infoPanel.add(new JLabel(String.format("%,.0f VND", hd.getTongTien())));
            ...
            btnInHoaDon.setBackground(Color.decode("#20C997"));
    ...
    private void inHoaDon(HoaDon hd, List<ChiTietHoaDon> chiTiet, KhachHang kh) {
        ...
        try (FileWriter writer = new FileWriter(fileName)) {
            ...
            for (ChiTietHoaDon cthd : chiTiet) {
                SanPham sp = sanPhamDAO.getSanPhamById(cthd.getSanPhamId());
                double thanhTien = cthd.getSoLuong() * cthd.getDonGia();
                writer.write(String.format("%-30s | %-10d | %-15.0f | %-15.0f\n", 
                    sp != null ? sp.getTen() : "N/A", cthd.getSoLuong(), cthd.getDonGia(), thanhTien));
            }

            writer.write("----------------------------------------\n");
            writer.write(String.format("Tổng tiền: %,.0f VND\n", hd.getTongTien()));
            writer.write("========================================\n");
